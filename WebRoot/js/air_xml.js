...
.ready(function(){
    ...
    success : function(xml){
	     ...
	     var id_str="";
		  $(xml).find('Item').each(function(i){
			  ...
			  $(this).find('Class').each(function(s){
				  ...
				  if(Seat=='A'){Seat='剩余票数'+'9'+'张以上'; } 
				 else if(Seat=='1' || Seat=='2' || Seat=='3' || Seat=='4' || Seat=='5' || Seat=='6' || Seat=='7' || Seat=='8'){Seat='剩余票数'+Seat+'张';}
				  else {Seat='';}				  
		...
		$str+="</tr>" ;		//退票改签标签                 
						 ...
						 if(lowprice > low_price  ){$num_low1=i; $num_low2=s; lowprice=low_price;}
						  }//if
				  })//read class
				  
				if(lowprice!=999999999){ $('#air_'+$num_low1+'_'+$num_low2).css('display',''); $('#air_'+$num_low1+'_'+$num_low2).attr('class','disp'+$num_low1); }	
				 $num_low1=0;
		         $num_low2=0;
				 lowprice=999999999;
				 low_price=0;
		    ...
		    if($count==0){$('#atb'+$count_i).remove(); $('#add_ip_air'+$count_i).remove(); $('#id_YCL'+$count_i).remove();}
			$("#loading").hide();//加载代码
		     //插入航班并去掉重复的航班
			 $str6="";
		    ...
		    for(var i=0;i<num_t1;i++){
			 ...
			 for(var j=i-1;j>=0;j--){
				 ...
				 if(input_air==input_air1){$("#add_ip_air"+j).remove();}
				 }}
			   }
		   //插入航班
		   //筛选航空公司
