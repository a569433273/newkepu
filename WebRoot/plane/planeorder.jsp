...
<script src="../js/base64decode.js"></script><!--解密净价票面价-->
<script src="../js/capture_air.js"></script> <!--接收预定信息-->
<script src="../js/secure.js"></script> <!--选中航意险select中option 时选中复选框航意险 并统计钱数-->
<script src="../js/add_sex.js"></script><!--单击护照 其他时 添加的信息 单击成人儿童时是否出现儿童价格-->
...
<script src="../js/checkbox_ture.js"></script><!--选中表单时 选中航意险-->
...
<script src="../js/update_delay.js"></script><!--选中延误险option时 改变价格 选中延误险checkbox-->
</head>
...
<!--预定信息-->
 <div class="res_top1">
  <table class="table1">
   ...
   <tr bgcolor="#f4fbfc" align="center"  height="30">
    <td width="13%" style="border-top:1px solid #dcf7fb; border-bottom:1px solid #dcf7fb; border-left:1px solid #dcf7fb; color:#0069ca; cursor:pointer;"  id="covert3" onclick="covert_table(this.id);"><span>退改签费用标准&nbsp;<img src="../images/up.jpg" style="margin-bottom:-5px;"/></span></td>
 ...
 <div class="top1"></div>
 <div class="top">
 <div style="float:left; line-height:30px; font-size:12px;">&nbsp;乘机人信息</div><div style="float:left;margin-top:5px; overflow:hidden;">&nbsp;<img src="../images/auto_write.gif" style="cursor:pointer;" class="showdiv" /></div><div style="float:left; line-height:30px; font-size:12px;"><span style="color:#999;">（请准确填写乘机人信息，以免在办理登机时发生问题）</span></div>
 </div>
 <!--信息部分-->
 <div class="top2" id="top2_info">
  <div class="t_1" id='add_p10'>
   ...
   <div class="t_1_top">
    ...
    <div style="float:left;"><span id="name_0"></span>&nbsp;<select class="younger" name="ag" onchange="yong_litter(this.id)" id="add_age0"><option checked="checked">成人(>12岁)</option><option>儿童(< 12岁)</option></select></div>
    </div>
    ...
    <div style="float:left; overflow:hidden; ">&nbsp;<select name="p_card" id="p_card0" class="which_1" onchange="set_sex(this.id)"><option>身份证</option><option>护照</option><option>其他</option></select></div>
    <div style="float:left;" id="card_disp0">&nbsp;<input type="text" name="p_name"  class="hz_val1" style="width:130px;" id="idCard_0" onkeyup="ck_idCard(this.id)" onfocus="ck_idCard1(this.id)" onblur="ck_ageCard(this.id)" /><span id="psg_card0"></span></div>
    ...
    <div style="float:left; oveflow:hidden;">&nbsp;<input name="" type="checkbox" value="" id="secur0" class="secur0"  onclick="box_ch(this.id)" checked="checked"/><span style="font-size:12px; color:#0068c9;" id="accid_s0"  onmousemove="mouse_disp(this.id);" onmouseout="mouse_hid()" >&nbsp;航意险</span><span style="font-size:12px;"><span style="color:#F00">¥30</span>/份</span></div>
    <div style="float:left; oveflow:hidden;"><select name="secure" id="secure0"  class="f_sec0" onchange="secur(this.id)"><option value="1" checked="checked">1份</option><option value="0">0份</option><option value="2">2份</option></select>&nbsp;</div>
    <div style="float:left; overflow:hidden; font-size:12px; line-height:20px;">保额最高100万/份</div>
    <div style="float:left; line-height:20px; margin-left:5px; overflow:hidden; cursor:pointer;" id="add_pf0" onclick="remov_pinfo(this.id)"><img src="../images/wrong.jpg" style="margin-top:5px;"></div>
   </div>
   <!--t_1_top1-->
    <div class="t_1_top1">
     <div style="float:left; width:710px;">     
    <!--点南儿童应该显示的信息-->
     ...
     <!--点南儿童应该显示的信息 is over-->
     <!--点击护照时要添加的信息-->
     <div style="float:left; width:450px; display:none;" id="disp_sex0">
      ...
      <div style="float:left;">&nbsp;<input type="text" name="date_add" class="input_val2" style="width:80px; color:#999;" value="yyyy-mm-dd"  id="dt_input0" onkeyup="ck_dtime(this.id)" onfocus="set_val(this.id)"  onblur="age_d(this.id)" /><span id="ck_tm0"></span></div>
      <div style="float:left; overflow:hidden; font-size:12px; line-height:20px;">&nbsp;<span style="color:#f00;">如：2014-09-18</span></div>
     </div>
     <!--点击护照时要添加的信息 over-->
     ...
     <div style="float:left;">&nbsp;<input name="" type="checkbox" value=""  id="delay_cl0" class="delay_cl" onclick="desc_sec(this.id)"/></div>
      <div style="float:left; overflow:hidden; line-height:20px; "><span style="font-size:12px; font-weight:bold; color:#0068c9;" id="delay_s0"  onmousemove="mouse_disp(this.id);" onmouseout="mouse_hid()">&nbsp;延误险</span><span style="color:#F00; font-size:12px;">¥20</span><span style="font-size:12px;">/份</span></div>
      <div style="float:left; oveflow:hidden;"><select  name="del_sec" id="del_sec0"  class="f_del0" onchange="del_upd(this.id)"><option value="0" checked="checked">0份</option><option value='1' >1份</option></select>&nbsp;</div>
      <div style="float:left; overflow:hidden; font-size:12px; line-height:20px;">保额最高100万/份</div>    
    </div>
   </div>
 </div>
 ...
 <!--添加按钮-->
 <div class="add_info"><div style="float:left; width:93px; height:21px; margin-top:10px; cursor:pointer;" onclick="add_pinfo();"><img src="../images/1_11.jpg" /></div><div style="float:left; color:#f00; font-size:12px; margin-top:15px;" id="no_add"></div><div style="float:left; display:none;" id="seat_hide"></div></div>
  <!--联系人信息部分-->
  <div class="peo1_info">
   <div class="top1"></div>
   <div class="top">
     <div style="float:left; line-height:30px; font-size:12px;">&nbsp;联系人信息</div><div style="float:left;margin-top:5px; overflow:hidden;">&nbsp;<img src="../images/auto_write.gif" style="cursor:pointer;" class="showdiv" /></div><div style="float:left; line-height:30px; font-size:12px;"><span style="color:#999;">（请准确填写乘机人信息，以免在办理登机时发生问题）</span></div>
    </div>
   <div class="top2">
...
<!--提交订单-->
<!--航空隐藏部分-->
 <input name="air_Name" type="hidden"  id="air_Name" value="" /> <!--航空名称-->
...
<div class="showbox">
   ...
   <div class="mainlist" >
      ...
      <div class="lg5"><p style="text-align:left; margin-left:30px;"><input type="image" src="../images/submit_img.png"/></p></div>
